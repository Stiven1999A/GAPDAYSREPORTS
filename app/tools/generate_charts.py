...
from dataprocessing import load_data, preprocess_data, prepare_data_weekly_chart
...
def stacked_bar_chart(df: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(
        ...
        yaxis=dict(
            tickformat=".0f",
            ticktext=[f"{int(h):02d}h:{int((h % 1) * 60):02d}m" for h in range(int(max(df[CHART_COLUMNS].sum(axis=1))) + 2)],
            tickvals=list(range(int(max(df[CHART_COLUMNS].sum(axis=1))) + 1))
        ),
        legend=dict(
            orientation="h",
            yanchor="top",
    ...
    # Update bar traces with custom hover templates
    for i, col in enumerate(CHART_COLUMNS):
        fig.data[i].hovertemplate = f"<b>{col}</b><br>Week: %{{x}}<br>Hours: %{{y:.2f}}m<extra></extra>"
    
    # Update line traces with custom hover templates
    fig.data[len(CHART_COLUMNS)].hovertemplate = "<b>Daily Productive Average</b><br>Week: %{x}<br>Hours: %{y:.2f}<extra></extra>"
    fig.data[len(CHART_COLUMNS) + 1].hovertemplate = "<b>Weekly Productivity Accumulated Average</b><br>Week: %{x}<br>Hours: %{y:.2f}<extra></extra>"

    return fig

#print(prepare_data_weekly_chart(preprocess_data(load_data(alchemy_connection(), pd.to_datetime('2025-11-28')))))
